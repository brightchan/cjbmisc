...
#' @param feat.plot vector of the features to be plotted
...
plot_feat_subtype <- function(plotdf,subtype,feat.plot,cont.plot="boxplot",
                              cont.test="wilcox.test",cont.show.mean=FALSE,show.count=TRUE,
                              disc.col=comp_hm_colist_full, cont.col=comp_hm_colist_full$disc,
                              disc.width=0.8,cont.width=0.9,
                              cont.anno.vjust=0.2,disc.anno.vjust=0.04,
                              aspect.ratio=1.4){
  ...
  para.plot <- lapply(feat.cont,function(x){ #parameter for continuous plots
    ...
    return(c(min(dat,na.rm=TRUE)-1.5*adj,max(dat,na.rm=TRUE)+2.5*adj,adj)) #the bottom, the top of the plot, and the adj value
  }) %>% setNames(feat.cont)
  ...
  plot.list <- lapply(feat.plot,function(x){
    if(x %in% feat.disc){
      ...
      p <- perc_barplot(plotdf,subtype,x,title=x,show.count=show.count,width=disc.width,
                        col=col,xlab="",ylab="",anno.vjust=disc.anno.vjust,aspect.ratio=aspect.ratio)
    } else {
      ...
      } else p <- p+stat_compare_means(method=cont.test)
